...
const Types = ({ data }: { data: any }) => {
  ...
  const getData = async () => {
    try {
      const response = await axios.get('/admin/filters/retrives'); // Adjust the endpoint as necessary
      return response.data.jobTypes || []; // Adjust based on the response structure
    } catch (error) {
  ...
  React.useEffect(() => {
    setLatestData(data); // Update latestData whenever data changes
  }, [data]);

  const addTags = async (value: { type: string }) => {
    const capitalizedType = capitalize(value.type);
    console.log("Attempting to add job type:", capitalizedType);
    console.log("Current tags:", tags);

    const latestData = await getData(); // Assuming getData() fetches the latest job types

    // Check if the job type already exists in latestData or tags
    if (
      latestData.includes(capitalizedType) ||
      tags.includes(capitalizedType)
    ) {
      setError("type", {
        type: "manual",
      ...
      setTags((prevTags: any) => {
        const newTags = [...prevTags, capitalizedType];
        console.log("New tags after addition:", newTags);
        return newTags;
      });
      reset(); // Reset the form after adding the tag
    }
  };

  const addAllTags = () => {
    setProcessing(true);
    const newTags = [...tags, ...(Array.isArray(data) ? data : [])];
    console.log("Reached heree ee e eee", newTags);
    if (!_.isEqual(newTags, data)) {
      try {
        console.log("Reached heree ee 2222", newTags);
        authAxios({
          method: "POST",
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
      <div>
        <h3 className="text-xxs">Create Job Type</h3>
        {/* Add job types form */}
