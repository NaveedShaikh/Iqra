...
import ViewerCardBroadcaster from "../VideoCards/viewerCardBroadcaster";
...
const VideoLayoutBroadcaster: React.FC<VideoLayoutProps> = ({
  broadcaster,
  viewers,
  broadcastedStream,
  toggleEnhance,
  ...
  return (
    <div className="w-full h-full primary-background-color grid grid-cols-4 grid-rows-4 gap-5">
      {/* Top row */}
      {displayedViewers.slice(0, 4).map((viewer: any, index: number) => (
        <ViewerCardBroadcaster role={role} key={index} viewerObject={viewer} />
      ))}
      {displayedViewers.slice(4, 8).map((viewer: any, index: number) => (
        <div
          key={index + 4}
          className="secondary-background-color text-center rounded-lg overflow-hidden"
        >
          <ViewerCardBroadcaster role={role} viewerObject={viewer} />
        </div>
      ))}
      {/* Middle section */}
      ...
      {/* Bottom row */}
      {displayedViewers.slice(8, 12).map((viewer: any, index: number) => (
        <div
          key={index + 8}
          className="secondary-background-color text-center rounded-lg overflow-hidden"
        >
...
export default VideoLayoutBroadcaster;
