...
export async function GET(request: Request) {
  try {
    ...
    const events = await connect.getBlogsPrivate(accessToken);
...
export async function POST(request: NextRequest) {
  try {
    ...
    const blogData = {
      ...
      blogContent: data.get("blogContent"),
    };
    ...
    console.log("event data ", blogData);
    const blog = await connect.postBlog(accessToken, blogData, images);
